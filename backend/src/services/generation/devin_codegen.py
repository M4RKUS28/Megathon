...
def _build_prompt(spec: dict, asset_map: dict) -> str:
    ...
    return (
        ...
        "- Mandatory subagent workflow: spin a separate subagent for EVERY PAGE (not just "
        "per chapter). Each page subagent receives the page spec and must implement that "
        "page's complete React component — layout, blocks, interactions, media players, "
        "animations and accessibility. Run ALL page subagents CONCURRENTLY so they execute "
        "in true parallelism. After all page subagents finish, integrate their components "
        "into each chapter's index and wire up the router. Do not skip subagents for short "
        "or simple pages — every page gets its own subagent.\n"
        ...
        "- Rich text in blocks: block `text`, `items`, headings and list entries may "
        "contain inline HTML tags (`<b>`, `<i>`, `<a>`, `<code>`, `<br/>` etc.). Always "
        "render them as real HTML (use `dangerouslySetInnerHTML` or a sanitising renderer "
        "like DOMPurify) — never display raw tags as literal text.\n"
...
def _build_chapter_prompt(spec: dict, chapter: dict, chapter_index: int) -> str:
    ...
    return (
        ...
        "- Mandatory subagent workflow: spin a separate subagent for EVERY PAGE in this "
        "chapter. Each page subagent builds that page's complete React component with all "
        "blocks, interactions, media players and animations. Run ALL page subagents "
        "CONCURRENTLY for true parallelism. After they finish, wire the page components "
        "into the chapter index. Do not skip subagents for any page.\n"
        ...
        "- Rich text: block `text`, `items` and list entries may contain inline HTML "
        "(`<b>`, `<i>`, `<a>`, `<code>`, `<br/>` etc.). Render them as real HTML "
        "(use `dangerouslySetInnerHTML` or DOMPurify) — never show raw tags as text.\n"
