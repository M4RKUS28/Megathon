...
MinIO + Redis (Arq worker)**, served behind Nginx.
...
```
                       ┌──────────────┐      REST       ┌───────────────┐
   Browser (SPA) ──────▶   Nginx :80  ├────────────────▶│  FastAPI :8000 │
   React + Vite        └──────┬───────┘                 └───────┬────────┘
...
```
.
├── deploy/                          # compose files (symlinked from repo root)
...
├── nginx/                           # routing (api, SPA, /storage course hosting)
...
Hosted courses are served by Nginx at `/storage/courses/<tenant>/<courseId>/v<n>/index.html`.
...
```bash
cp .env.example .env
# edit .env — domains, passwords, and any provider keys you have (all optional)
